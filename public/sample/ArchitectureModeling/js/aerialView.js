...
AerialView.prototype = {
	...
	setWindowWidthHeight : function(id) {
		var parentObj = parent.document.getElementById('frameObj') ? parent.document.getElementById('frameObj') : parent.document.getElementById('embedObj'); 
		...
		if (id) {
			try {
				...
				if (Math.abs(parent.document.body.offsetHeight - parentObj.offsetHeight) > 10 || Math.abs(parent.document.body.offsetWidth - parentObj.offsetWidth) > 10) {
					parentObj.style.width = parent.document.body.offsetWidth * 0.8;
					parentObj.style.height = parent.document.body.offsetHeight * 0.95;
				}
			} catch(ex) {
		...
		if (this.isOpen()) {
			this.position(cw - 201 - this.dWidth, 1);
		} else {
			this.position(cw - 201 - this.dWidth, 1);
		}
		return this;
