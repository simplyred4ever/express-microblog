...
Tree.prototype.create = function () {
	...
	default: //所有条目
		...
		this.text = new $$.Text(this.id + 'text', this.a.self).position(x + stateIconWidth + 2, y + ((childNum + 1) * nodeHeight) + nodeHeight / 2 + 8).text(this.name, textNumber)
			.attr('onclick', 'parent.openWin("'+ [this.exabs.id, this.exabs.mainDocId, this.exabs.docEdtionId].join(',') +'")').addClass("aColor text");
	}
...
DataMapping.prototype.create = function (fromNodeId, fromId, toNodeId, toId, color) {
	...
	this.line = new $$.Line(fromId + "_" + toId, 'lineWorkspace').position(xy).addClass('line').css('fill', color).css('stroke', color).css('marker-end', 'url(#arrow_' + color + ')');
