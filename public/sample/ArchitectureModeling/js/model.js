 ...
 * @param {Object} name
 */
Model.SuperNode = function (id, parentId, name) {
	this.id = id;
	...
	this.group = new $$.Group(id, $$.$(parentId));
...
Model.SuperNode.prototype.size = function (width, height) {
	if (arguments.length === 0) {
		...
		this.ext.width = $$.round(width, 20);
		this.ext.height = $$.round(height, 20);
		return this;
	}
};
/**
...
Model.SuperNode.prototype.draw = function () {
	// g标签
	this.group.position(this.ext.x, this.ext.y).addClass('DragNode');

	// 主图形
	this.shape.bound(0, 0, this.ext.width, this.ext.height)
	.css('fill', this.ext.backgroundColor)
	.addClass('node');

	// 文字
	this.text.position(this.ext.width / 2, 20)
...
Model.SystemNode = function (id, parentId, name, ext) {
	Model.SystemNode.superClass.constructor.call(this, id, parentId, name);
	this.ext = ext;
	this.ext.nodeType = 'System';
	this.shape = new $$.Rect(id + 'r', this.group.self);
	this.text = new $$.Text(id + 't', this.group.self);
	this.draw();
	// 提示点
	//this.addGuide();
};
...
Model.ActorNode = function (id, parentId, name, ext) {
	Model.ActorNode.superClass.constructor.call(this, id, parentId, name);
	this.ext = ext;
	this.ext.nodeType = 'Actor';
	this.shape = new $$.Ellipse(id + 'r', this.group.self);
	this.text = new $$.Text(id + 't', this.group.self);
	this.draw();
	// 提示点
	//this.addGuide();
...
Model.OrganizationNode = function (id, parentId, name, ext) {
	Model.OrganizationNode.superClass.constructor.call(this, id, parentId, name);
	this.ext = ext;
	this.ext.nodeType = 'Organization';
	this.shape = new $$.Hexagon(id + 'r', this.group.self);
	this.text = new $$.Text(id + 't', this.group.self);
	this.draw();
	// 提示点
	//this.addGuide();
...
Model.ServiceNode = function (id, parentId, name, ext) {
	Model.ServiceNode.superClass.constructor.call(this, id, parentId, name);
	this.ext = ext;
	this.ext.nodeType = 'Service';
	this.shape = new $$.Pentagon(id + 'r', this.group.self);
	this.text = new $$.Text(id + 't', this.group.self);
	this.draw();
	// 提示点
	//this.addGuide();
...
Model.AllEdge = {
	...
	importJson : function (edge) {
		for (var i = 0; i < edge.length; i++) {
			...
			new Model.Edge(edge[i].id, 'edgeGroup', edge[i].source, edge[i].target, edge[i].sourcePort, edge[i].targetPort, edge[i].ext).syncEndPoint().draw(true);
		}
		return this;
	}
}
 ...
 * 资源流
 */
Model.Edge = function (id, parentId, source, target, sourcePort, targetPort, ext) {
	this.id = id;
	...
	this.shape = new $$.Polyline(id, $$.$(parentId));
	this.ext = ext;
...
Model.Edge.prototype.draw = function (auto) {
	...
	if (auto) {
		if (this.ext.points.length === 2) {
			...
			d = Math.abs(sp.cx - point.x) >= Math.abs(sp.cy - point.y);
			if (sp.cx < point.x && d) {
				this.sourcePort = 'e';
			} else if (sp.cy > point.y && !d) {
				this.sourcePort = 'n';
			} else if (sp.cx > point.x && d) {
				this.sourcePort = 'w';
			} else if (sp.cy < point.y && !d) {
				this.sourcePort = 's';
			}
			point = this.ext.points[this.ext.points.length - 2];
			d = Math.abs(point.x - tp.cx) >= Math.abs(point.y - tp.cy);
			if (point.x < tp.cx && d) {
				this.targetPort = 'w';
			} else if (point.y > tp.cy && !d) {
				this.targetPort = 's';
			} else if (point.x > tp.cx && d) {
				this.targetPort = 'e';
			} else if (point.y < tp.cy && !d) {
				this.targetPort = 'n';
			}
	...
	this.shape.setPoints(this.ext.points).addClass('resourceFlow');
	return this;
}
/**
 * 同步两端坐标
