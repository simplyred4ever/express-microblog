...
TopoTree.prototype._drawTree = function () {
	...
	for (var n = 0; n < this.nDatabaseNodes.length; n++) {
		...
		// 标题
		var rect = new $$.Rect(node.id, 'workspace').bound(node.XPosition, node.YPosition, node.w, node.h).addClass('node').attr('title', node.dsc);//.attr('onclick', this.obj + '.selectNode(\'' + node.id + '\', true);'); //.css('stroke', border).css('fill', color);
		...
		for (var e = 0; e < node.entries.length; e++) {
			item = node.entries[e];
			rect = new $$.Rect(item.id, 'workspace').bound(node.XPosition, node.YPosition + this.config.defaultNodeHeight * (e + 1), node.w, this.config.defaultNodeHeight).addClass('entry').attr('title', item.name); //.css('stroke', border).css('fill', color);
			if (node.nodeParent === this.root) {
				rect.addClass('current');
			} else {
				rect.removeClass('current');
			...
			text.attr('onclick', 'parent.showItem("'+ item.url +'")');
	...
	for (var n = 0; n < this.nDatabaseNodes.length; n++) {
		...
		for (var i = 0; i < this.mapping.length; i++) {
			...
			line = new $$.Line(sourcePort.id + '_' + targetPort.id, 'lineWorkspace').position({
				...
				}).addClass('line').css('stroke', color).css('marker-end', 'url(#arrow_' + color + ')');
